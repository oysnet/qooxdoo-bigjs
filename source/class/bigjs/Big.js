...
qx.Class.define("bigjs.Big",
{
  ...
  */
  ...
  {
    this.base(arguments);
    this._big = new Big(value);
  },
  members : {
    _big : null,
    cmp : function(y) {
      var retr = this._big.cmp(y._big);
      return new bigjs.Big(retr);
    },
    div : function(y) {
      var retr = this._big.div(y._big);
      return new bigjs.Big(retr);
    },
    minus : function(y) {
      var retr = this._big.minus(y._big);
      return new bigjs.Big(retr);
    },
    mod : function(y) {
      var retr = this._big.mod(y._big);
      return new bigjs.Big(retr);
    },
    plus : function(y) {
      var retr = this._big.plus(y._big);
      return new bigjs.Big(retr);
    },
    pow : function(e) {
      var retr = this._big.pow(e);
      return new bigjs.Big(retr);
    },
    round : function(dp, rm) {
      var retr = this._big.round(dp, rm);
      return new bigjs.Big(retr);
    },
    sqrt : function() {
      var retr = this._big.sqrt();
      return new bigjs.Big(retr);
    },
    times : function(y) {
      var retr = this._big.times(y._big);
      return new bigjs.Big(retr);
    },
    toString : function() {
      var retr = this._big.toString(arguments);
      return retr;
    },
    valueOf : function() {
      var retr = this._big.valueOf(arguments);
      return retr;
    },
    toExponential : function(dp) {
      var retr = this._big.toExponential(dp);
      return retr;
    },
    toFixed : function(dp) {
      var retr = this._big.toFixed(dp);
      return retr;
    },
    toPrecision : function(sd) {
      var retr = this._big.toPrecision(sd);
      return retr;
    }
  }
});
